...
from imblearn.over_sampling import SMOTE #pour le SMOTE
...
class Pretraitement: 
	...
	def epuration_donnees(data):
		...
		print("Nombre de données avant traitement : ",nombre_l)
		...
		print("Nombre de données après traitement avant SMOTE : ",nombre_lignes)
		...
		#Ici on applique la méthode du SMOTE
		sm = SMOTE(random_state=0)
		y = data.Cover_Type #donnees 
		X=data.drop('Cover_Type',axis=1) #classes
		
		data_train,target_train = sm.fit_resample(X,y) #on remet la taille à jour en fonction de la taille de X et y avant traitement des données
